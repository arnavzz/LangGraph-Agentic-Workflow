...
def execute_task(state):
    """Executes the current task, updates the task list dynamically."""
...
def generate_tasks(instruction, target):
    ...
    try:
        return json.loads(response.content)  # Parse AI response
    except:
        return [{"step": 1, "tool": "nmap", "command": f"nmap -Pn -p- {target}"}]  
